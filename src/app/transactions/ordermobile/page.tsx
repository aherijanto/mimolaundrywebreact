...
export default function OrderMobilePage() {
  ...
  useEffect(() => {
    fetch('/api/transactions?order_from=android')
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 w-full">
      <div className="flex items-start">
        <h1 className="text-2xl font-bold m-8 text-blue-600">Orders From Mobile</h1>
      </div>
                ...
                <table className="min-w-full w-full text-sm">
                  ...
                  <tbody>
                    {transactions.map((t, idx) => (
                      <tr key={t.id} className="odd:bg-white even:bg-gray-50 transition hover:bg-blue-50">
                        ...
                        <td className="px-3 py-2 text-center text-blue-600 font-semibold">
                          {t.payment_status ? (
                            ...
                            <span className="bg-red-500 text-white px-2 py-1 rounded">Unpaid</span>
                          )}
                        </td>
                      </tr>
                    ))}
